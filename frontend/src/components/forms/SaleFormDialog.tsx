...
export function SaleFormDialog({ open, onOpenChange, onSubmit }: SaleFormDialogProps) {
  ...
  useEffect(() => {
    if (selectedProduct && formData.quantity > selectedProduct.inStock) {
      setFormData(prev => ({ ...prev, quantity: Math.max(0.5, selectedProduct.inStock) }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.productId || formData.quantity < 0.5) {
      toast({ title: 'Validation Error', description: 'Please select a product and enter at least 0.5 quantity', variant: 'destructive' });
      return;
    }
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({ ...formData, quantity: Number.isFinite(quantity) ? quantity : 0 });
                  }}
                />
              </div>
