...
export function VisitFormDialog({ open, onOpenChange, onSubmit }: VisitFormDialogProps) {
  ...
  const defaultLocalMrSet = useRef(false);
  const defaultLocalMrId = localMRs[0]?.id || '';
  
  useEffect(() => {
    if (open && defaultLocalMrId && !defaultLocalMrSet.current) {
      defaultLocalMrSet.current = true;
      setFormData(prev => ({ ...prev, localMrId: defaultLocalMrId }));
    }
    if (!open) {
      defaultLocalMrSet.current = false;
    }
  }, [open, defaultLocalMrId]);
