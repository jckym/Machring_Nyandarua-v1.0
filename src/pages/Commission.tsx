...
export function Commission() {
  ...
  const exportExcel = () => {
    let csv =
      'Local MR,Manager,Total TOTs,Active TOTs,Total Sales,Total Commission\n';

    localMRSummaries.forEach(mr => {
      csv += `"${mr.name}","${mr.manager}",${mr.totalTots},${mr.activeTots},${mr.totalSales},${mr.totalCommission}\n`;
    });
  ...
  const exportPDF = () => {
    let text = `COMMISSION REPORT\n\n`;

    localMRSummaries.forEach(mr => {
      text += `${mr.name}\n`;
      text += `Manager: ${mr.manager}\n`;
      text += `Sales: ${formatCurrency(mr.totalSales)}\n`;
      text += `Commission: ${formatCurrency(mr.totalCommission)}\n\n`;
    });

    const blob = new Blob([text], { type: 'text/plain' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'commission_report.txt';
    link.click();
